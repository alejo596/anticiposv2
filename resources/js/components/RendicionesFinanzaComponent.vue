<template>
	<div>
			...
			<div class="card-body ">
				...
				<v-client-table :columns="columns_anticipos" v-model="data_anticipos" :options="options_anticipos">	
					...
					<div slot="enviar" slot-scope="props" >
						...
						@click="enviarrendicionfinanza(props.row.id,props.row.monto,props.row.cc_id,props.row.run,props.row.nombre,props.row.ultima_rendicion,props.row.id_solicitud)" :disabled="props.row.estado == 11">Enviar Anticipo</button>
					</div>
		...
		<!-- Modal -->
		<div class="modal fade " tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" id="form_rendicion" aria-hidden="true" >
			<div class="modal-dialog modal-xl" style="max-width: 1500px !important;">
				<div class="modal-content">
					<div class="modal-header">
						...
						<label class="border border-primary rounded">Monto Rendicion actual: ${{formatPrice(monto_total_rendiciones)}}</label><label class="border border-danger rounded">Monto Rendicion Total: ${{formatPrice(monto_total_rendicionestotal)}}</label>
						...
						<div class="row mt-2">
							...
							<div class="col-4">
								...
								<div class="result_producto" v-for="book_producto in newBooks_producto2" >
									<span @click="selectResult_producto(book_producto.produCodInt,book_producto.produNombre)" class="elecciondeproducto" @mousedown.prevent>
										{{ book_producto.produNombre}}
									</span>
								</div>
							</div>	
					...
					<div class="row mt-2">
						...
						<div class="col-4">
							...
							<div class="result_producto border border-success rounded" v-for="book_producto in newBooks_proveedor2" >
								<span class="offset-5 elegirproveedor" @click="selectResult_proveedor(book_producto.id,book_producto.nombre,book_producto.rut,book_producto.apellido_paterno,book_producto.apellido_materno,book_producto.tipo_de_empresa,book_producto.razon_social)" @mousedown.prevent>
			...
			<v-client-table :columns="columns_anticipos_2" v-model="data_anticipos_2" :options="options_anticipos_2">				
				...
				<div slot="editar" slot-scope="props">
					...
					<div >
						...
						<button class="btn btn-primary" :disabled="validated" @click="modaleditar(props.row.id,props.row,props.row.producto_id,props.row.produNombre)"> Editar</button>
					</div>
				</div>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<v-client-table :columns="columns_rendiciones" v-model="data_rendiciones" :options="options_Rendiciones">	
					<div slot="url" slot-scope="props">
						<a :href="props.row.url" target="_blank"><img :src="'assets/img/pdf.png'" width="50" height="50"></a>
	...
	export default {
		...
		data(){
			...
			return {
				...
				modalproducto:'',
				arramodaleditar:[],
				monto_total_rendicionestotal:0,
				nuevo_cpp:'',
				descripcion_docu:'Compra de productos',
				botonenviar:true,
				botonenviar2:false,
				...
				bloqueo:false,
				columns_anticipos_2: ['id', 'created_at', 'nombre','numero_doc','monto','cantidad_producto','nombre_estado','produNombre','cpp_num','url','editar','eliminar'],
				data_anticipos_2:[],
				options_anticipos_2: {
		...
		watch: {
			...
			search_producto(val) 
			{
				...
				if(val.length > 3) { 
					...
					axios.get('productos_aba/'+val).then(response=>{
		...
		methods:{
			...
			{
				this.bloqueo=true;
				axios.get('persona_anticipo/'+this.run_funcionario).then(resp=>{
			...
			asignar(id,fiador)
			{
				...
				axios.get('persona_anticipo/'+this.id_session).then(response=>{
			...
			enviarRendicion(id,monto,cc,run_fiador,nombre_fiador,id_solicitud)
			{
					...
					if (result.isConfirmed) {
						...
						axios.get('rendiciones_fondo/'+id).then(response=>{
							...
							for (var i = 0; i < response.data.length; i++) {
								...
								if(response.data[i].tipo_doc != 2)
								{
									this.monto_mas_monto_rend=response.data[i].monto;
						...
						setTimeout(()=>{
							Swal.fire({
								title: 'Desea enviar la rendición por el monto de: $'+this.formatPrice(this.monto_mas_monto_rend),          
								icon: 'warning',
								showCancelButton: true,
								confirmButtonColor: '#3085d6',
								cancelButtonColor: '#d33',
								...
								if (result.isConfirmed) {
									...
									var mesActual = fecha.getMonth() + 1; 
									var d=new FormData();
									d.append('monto',this.monto_mas_monto_rend);
										...
										axios.post('cpp_anticipos',anti).then(responseanticipos=>{
											var ins=this;
												...
												var anti={
													id:id,
													estado:11,
													id_cpp:this.nuevo_cpp,
													_method:'patch'
												}
												axios.post(url_ant,anti).then(respanticipo=>{
													var firmaup='firma_anticipo/'+id;
													var firmaupdata=
													{
														id_anticipo:id,
														firma:this.id_session,
														estado:1,
														unidad_negocio:6,
														_method:'patch'
													...
													axios.post(firmaup,firmaupdata).then(respupdatefirma=>{
                    		...
                    		Swal.fire({
                    			...
                    			if (result.isConfirmed) {
                    				...
                    				axios.get('rendiciones_fondo/'+id).then(response=>{
										...
										this.updatefondofijo(id,this.nuevo_cpp);

										axios.post(firmaup,firmaupdata).then(respupdatefirma=>{

											var firma1=new FormData();
											firma1.append('usuario','');
											firma1.append('estado',0);
											firma1.append('unidad',6);
											firma1.append('id_anticipo',id);

											axios.post('firma_anticipo',firma1).then(firma=>{
...
updatefondofijo(id,cpp)
{
	...
	axios.post(url_ant,anti).then(respanticipo=>{}).catch(err=>{});
	...
	if(this.id_proveedor =='0')
	{
		...
		axios.post('proveedor_anticipos',a).then(response=>{
			...
			axios.post('rendiciones_fondo',d).then(response=>{
				...
				this.getRendicionesfinanza(this.titulomodal);
...
sub:function(e)
{
	let reader = new FileReader();
            ...
            selectResult_producto(id,nombre)
            {
            ...
            getAnticipos()
            {
            	...
            	axios.get('fondo_fijo/'+this.id_session).then(response=>{
            		...
            		this.bloqueo=false;
            		this.dulcealert('error anticipos:'+err,'error');
            	});
            },
            ...
            getRendicionesfinanza(id)
            {
            	this.bloqueo=true;
            	axios.get('rendiciones_fondo2/'+id).then(response=>{
            		 
            		this.data_anticipos_2=[]
            		this.rendiciones=response.data;
            		this.monto_total_rendiciones=0;
            		this.monto_total_rendicionestotal=0
            		if(this.rendiciones.length == 0){
                                   this.bloqueo=false;
                             }
            		for (var i = 0; i < this.rendiciones.length; i++) {
            			this.monto_total_rendicionestotal=this.monto_total_rendicionestotal+this.rendiciones[i].monto;


            			if(this.rendiciones[i].estados == 1){
                        this.data_anticipos_2.push(response.data[i]);
                        this.monto_total_rendiciones=this.monto_total_rendiciones+this.rendiciones[i].monto;
            			}

            		
                             if(i == this.rendiciones.length-1 ){
                                   this.bloqueo=false;
                             }
